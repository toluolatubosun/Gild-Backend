...
class UserService {
    ...
    async getUser({ username, userId }: GetUserInput) {
        if (userId) {
            return await this.getOne(userId);
        } else if (username) {
            return await this.getByUsername(username);
        } else {
            throw new CustomError("UserId or Username is required");
        }
    }
    ...
    async addStripeAccountId(userId: string, stripeAccountId: string) {
        const user = await User.findOneAndUpdate({ _id: userId }, { $set: { stripeAccountId } }, { new: true });
        if (!user) throw new CustomError("user does not exist");
