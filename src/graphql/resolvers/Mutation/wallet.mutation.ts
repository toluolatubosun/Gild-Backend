...
export default {
    ...
    walletResendTransferOTP: async (_: any, { receiverId, amount }: InitializeTransferArgs, context: Context): Promise<Boolean> => {
        const user = guard(context.user, ROLE.USER);
        return await WalletService.resendTransferOTP(user.id, receiverId, amount);
    },
